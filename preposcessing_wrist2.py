...
def generate_data2(folder_path, num_of_measurement, shape=(128, 256)):
    ...
    for i in tqdm(range(len(data))):
        ...
        parent_folder = folder_path + "measurements/" + str(num_of_measurement) + "_" + str(shape[0]) + "_" + str(
            shape[1])
        if not os.path.exists(parent_folder):
            os.makedirs(parent_folder)
        # save the measurements to csv file
        csv_save_path = folder_path + "measurements/" + str(num_of_measurement) + "_" + str(shape[0]) + "_" + str(
            shape[1]) + "/" + str(i) + ".csv"
