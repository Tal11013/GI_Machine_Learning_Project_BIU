...
def generate_data(folder_path, num_of_measurement, shape=(64, 128)):
    ...
    reference_matrix = np.random.randn(num_of_measurement, shape_tuple[0] * shape_tuple[1])
    ...
    for i in tqdm(range(len(data))):            # clear cache and unused memory from the GPU
        ...
        img = img.resize(size=shape_tuple, resample=Image.BILINEAR)
        ...
        if torch.cuda.is_available():
            ...
            measurements = torch.matmul(reference_matrix_cuda, img.view(shape_tuple[0] * shape_tuple[1], 1)).cpu().numpy()
        else:
            # multiply the reference matrix with the img
            measurements = np.matmul(reference_matrix, img.reshape(shape_tuple[0] * shape_tuple[1]))
        # save the measurements to csv file
        pd.DataFrame(measurements).to_csv(folder_path + "measurements/"+str(num_of_measurement) + "_"+str(shape_tuple[0]) +
